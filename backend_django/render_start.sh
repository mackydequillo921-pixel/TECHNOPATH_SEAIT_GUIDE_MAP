...
echo "from apps.users.models import AdminUser; AdminUser.objects.filter(username='admin').exists() or AdminUser.objects.create_superuser('admin', 'admin@example.com', 'admin123')" | python manage.py shell
